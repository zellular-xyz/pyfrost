...
for key in keys:
    round2_send_data, round2_save_data = key.round2(
        round1_received_data, saved_data[key.node_id][dkg_id])
...
for key in keys:
    result = key.round3(
        round1_received_data, round2_received_data[key.node_id], saved_data[key.node_id][dkg_id])

saved_data['common_data'] = {}
saved_data['private_data'] = {}
for key in keys:
    nonces_common_data, nonces_private_data = __init__.nonce_preprocess(
        int(key.node_id))
...
group_sign = __init__.aggregate_signatures(msg, signs, Utils.code_to_pub(
