...
class Key:
    ...
    def sign(
        self, commitments_dict: Dict, message: str, nonces: List[Dict]
    ) -> Tuple[Dict, Dict]:
...
def verify_single_signature(
    ...
    public_key_share: int,
    single_signature: Dict[str, int],
    group_key: Point,
) -> bool:
    ...
    challenge = Web3.solidity_keccak(
        ...
        [
            Web3.to_int(hexstr=group_key_pub["x"]),
...
def aggregate_nonce(
    message: str, commitments_dict: Dict[str, Dict[str, int]], group_key: Point
):
