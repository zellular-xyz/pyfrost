...
def key_generate(nodes , threshold , private_key = None):
    ...
    aggregated_public_key = ECPublicKey(tss.curve.mul_point(n_inverse , public_fx))
    for node in nodes:
        key_shares.append({'i' : node , 'key' : ECPrivateKey(sum(f_share[node]) * n_inverse , tss.curve) , 'public_key' : aggregated_public_key }) # , 'publicKey' : total_Fx
    return key_shares
...
message_hash = int.from_bytes(Web3.solidity_keccak(['string'] , ["Hello every body"]),'big')
...
for key_share in key_shares:
    ...
    signatures.append({'index' : key_share['i'] ,'sign' : tss.schnorr_sign(share , nonce , public_nonce , message_hash)})
...
for i in range(5):
    ...
    aggregated_signature = tss.schnorr_aggregate_signatures(threshold , [s['sign'] for s in signates_subset] , [s['index'] for s in signates_subset])
    verified = tss.schnorr_verify(public_key , message_hash , aggregated_signature)
    print(f"Selected Nodes : {[s['index'] for s in signates_subset]} , verified : {verified}")
    ...
    for key_share in key_shares_subset:
        ...
        for nonce in private_nonces:
            if nonce['i'] == id:
                nonce_d = nonce['d']
                nonce_e = nonce['e']
        single_signature = tss.frost_single_sign(id , share , nonce_d , nonce_e , message , commitments_list , group_key)
        share_public_keys[id] = tss.pub_to_code(share.get_public_key())
        signatures.append(single_signature)

    group_sign = tss.frost_aggregate_signatures(signatures ,share_public_keys , message , commitments_list , group_key)
